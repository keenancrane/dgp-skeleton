...
namespace DDG
{
   ...
   void Mesh::buildDifferentialOperators( void )
   {
      // build basic DEC operators
      HodgeStar0Form<Real>::build( *this, star0 );
      HodgeStar1Form<Real>::build( *this, star1 );
      HodgeStar2Form<Real>::build( *this, star2 );
      exteriorDerivative0Form<Real>::build( *this, d0 );
      exteriorDerivative1Form<Real>::build( *this, d1 );

      // build composite operators
      delta1 = star0.inverse() * d0.transpose() * star1;
      delta2 = star1.inverse() * d1.transpose() * star2;
      Delta0 = delta1*d0;
      Delta1 = delta2*d1 + d0*delta1;
   }
}
